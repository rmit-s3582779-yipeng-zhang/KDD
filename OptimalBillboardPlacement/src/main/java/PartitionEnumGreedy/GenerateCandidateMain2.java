 ...
 */
public class GenerateCandidateMain2 {
    ...
    public static void main(String[] args) {
        ...
        testParameter.add(new TestParameter(50, 2, 1, 1, 0, true, false, true, 0.05));
        ...
        GenerateCandidateMain2 generateCandidate = new GenerateCandidateMain2();
    ...
    public void test(ArrayList<TestParameter> testParameters) {
        ...
        for (TestParameter testParameter : testParameters) {
            try {
                ...
                System.out.println("Budget " + budget + "," + " Trajectory " + trajectoryNumber + ", BillboardNumber " + multipleBillboardNumber * 500 + ", pow " + pow + ", THETA " + reta + ", CostFix " + costFixed);
                ...
                fileWriter.writeToFile("Budget " + budget + "," + " Trajectory " + trajectoryNumber + ", BillboardNumber " + multipleBillboardNumber * 500 + ", pow " + pow + ", Remove " + removeHighest + ", AddBillboard(" + miniBillboardNumber + ") "
                        + ifAddBillboardRandom + ", CostFix " + costFixed + "\r\n");
    ...
    private ArrayList<ClusterBillboard> getCandidate(List<Billboard> billboards, Boolean calculate[]) {
        ...
        int upperBoundCost = (int) greedyAlgorithm.greedy(getBillboard(billboards), 100000000).cost;
        ...
        if (upperBoundCost < maxBudget) {
            maxBudget = upperBoundCost;
        }

        for (int i = maxBudget; i >= chargeDensity; i -= chargeDensity) {
            if (i > maxCharge)
                break;
    ...
    private void getFixedCost(String city) {
        try {
            String root = System.getProperty("user.dir");
            //String root = GenerateCandidateMain.class.getProtectionDomain().getCodeSource().getLocation().getPath();
            ...
            if (Developer.SYSTEM.equals("Mac")) {
                fis = new FileInputStream("./"+clusterName+"/4w-trip-" + clusterNumber + "-Clusters.cluster");
            } else if (Developer.SYSTEM.equals("Win")) {
                fis = new FileInputStream(root + "\\"+clusterName+"\\4w-trip-" + clusterNumber + "-Clusters.cluster");
            } else if (Developer.SYSTEM.equals("Linux")) {
                fis = new FileInputStream(root + "/"+clusterName+"/4w-trip-" + clusterNumber + "-Clusters.cluster");
            }
    ...
    public List<List<Billboard>> getPartition(boolean combine, String city) {
        ...
        try {
            ...
            if (Developer.SYSTEM.equals("Mac")) {
                ...
                fis = new FileInputStream(root + "\\OptimalBillboardPlacement\\" + clusterName + "\\" + trajectoryNumber + "w-trip-" + clusterNumber + "-Clusters.cluster");
            } else if (Developer.SYSTEM.equals("Linux")) {
